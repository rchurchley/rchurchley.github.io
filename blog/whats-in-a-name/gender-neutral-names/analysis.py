...
def main():
    ...
    neu = n.girls.index.intersection(n.boys.index)
    nn = pd.concat({ 'girls': n.girls.reindex(neu).sum(axis=1), 'boys': n.boys.reindex(neu).sum(axis=1)}, axis=1)
    nn[nn.girls >= 50][nn.boys >= 50].to_csv("foo.csv")
